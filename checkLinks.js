...
async function checkLinksFromFile(filename, filterBroken) {
    ...
    try {
        ...
        const csvContent = brokenLinks
            .map((link) => `${link.url},${link.state === 'BROKEN' ? 'BROKEN' : 'OK'},${link.status},${link.parent}`)
            .join("\n");
        await fs.writeFile(outputFilePath, 'Link,Status,HTTP Code,Parent\n' + csvContent);

        console.log(`Detected ${brokenLinks.length} broken links out of ${links.length} total links.`);
    } catch (error) {
...
rl.question('Do you want to filter only broken links? (yes/no) [yes]: ', (answer) => {
    const filterBroken = answer.trim().toLowerCase() === 'yes' || answer.trim() === '';
    rl.close();
